document.addEventListener("DOMContentLoaded", function () {
    ...
    /* Если мультиселект не работает рекомендуется
    обращаться к Богу(у разработчика нервный срыв и маниакальная депрессия,
    возникшая при создании мультиселекта).*/

    var select = document.getElementById("points");
    for(var j = 0; j < groups.length; j++)
        addToSelect(groups[j].items);
    ...
    function addToSelect(arr){
        for(var i = 0; i < arr.length; i++)
        {
            ...
            option.setAttribute("value",arr[i].name);
            select.insertBefore(option,select.lastChild);
        }
    }

    /* Построение маршрута. */
    buildRoute.onclick = function buildRouteClick(){
        timeLimit = document.getElementById("timeLimit").value; // Ограничение по времени в минутах.
        addRecomendation = document.getElementById('addRecomendation').checked; 
        pointsName = getSelectValues(select); // Названия мест посещения, которые выбрал пользователь.

        /*
        document.getElementById("time").innerText = время прохождения маршрута;
        document.getElementById("lenght").innerText = длинна маршрута;
        */
    }
